...
const chatLogFile = path.join(dataDir, "chat_logs.json");
const intentsFile = path.join(dataDir, "intents.json");
...
// Initialize chat logs file if it doesn't exist
if (!fs.existsSync(chatLogFile)) {
  fs.writeFileSync(chatLogFile, '[]', 'utf8');
}

// Initialize intents file if it doesn't exist
if (!fs.existsSync(intentsFile)) {
  ...
  fs.writeFileSync(intentsFile, JSON.stringify(defaultIntents, null, 2), 'utf8');
}
